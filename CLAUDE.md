...
- Android-only data bits remain: `data/auth/` (DataStore `TokenStore` + `TokenRefreshAuth`), `data/image/` (`AndroidImageUploader`), `home/data/AndroidHomeFeedStrings`, and `auth/` (`DefaultAuthRepository`, `GoogleSignIn`).
...
The `auth` package (`AuthRepository`, `AuthViewModel`, `GoogleSignIn`, `ui/AuthScreens.kt`) handles email/password + Google sign-in (Credential Manager + `googleid`). Auth is a **short-lived access JWT + an opaque, revocable refresh token**; both are persisted in `data/auth/TokenStore` (DataStore). `data/auth/TokenRefreshAuth.kt` installs the Ktor client `Auth` (bearer) plugin so an expired access token is transparently refreshed on `401` (the refresh is serialized) and the request retried; logout revokes the refresh token server-side. Launch is gated on having a token. `BuildConfig.GOOGLE_WEB_CLIENT_ID` (from Gradle properties) being blank disables the Google button.
