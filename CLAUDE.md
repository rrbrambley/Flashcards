...
- Bundle id `com.rrbrambley.flashcards`, deployment target iOS 17, iPhone (`TARGETED_DEVICE_FAMILY = 1`), generated `Info.plist` (`GENERATE_INFOPLIST_FILE`). Sources under `iosApp/Flashcards/`; `Assets.xcassets` holds `AppIcon` (placeholder) + `AccentColor`.
- **Headless build/verify** (Claude can't drive the Xcode GUI): `xcodebuild -project iosApp/Flashcards.xcodeproj -scheme Flashcards -destination 'platform=iOS Simulator,name=iPhone 17' build`, then `xcrun simctl boot/install/launch` to confirm it runs. The Xcode SDK needs a matching simulator runtime — if missing, `xcodebuild -downloadPlatform iOS`.
